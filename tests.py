...
def run_test(test: TestCase) -> TestResult:
    ...
    try:
        ...
        log(f"[FAIL] {test.name} (Critical error)", InputColor.ERROR)
    ...
    if test.expected_print is not None and test.expected_print != program_result:
        ...
        log(f"       Expected: {shorten(repr(test.expected_print.strip()))} (len={len(test.expected_print)})", InputColor.WARNING)
        log(f"       Received: {shorten(repr(program_result.strip()))} (len={len(program_result)})", InputColor.WARNING)
        return TestResult.FAIL
...
def run_tests():
    ...
    if not prerequisite_flake8(file):
        log(f"[FAIL] File '{file}' does not meet PEP 8 standard.", InputColor.ERROR)
    ...
    log(f" Critical Errors: {tests_error}", InputColor.ERROR if tests_error > 0 else InputColor.SUCCESS)
    ...
    if tests_total == tests_passed:
        log("\n[PASS] All tests passed successfully.", InputColor.SUCCESS)
    else:
        log("\n[FAIL] Not all tests passed. Fix your code and try again.", InputColor.ERROR)
